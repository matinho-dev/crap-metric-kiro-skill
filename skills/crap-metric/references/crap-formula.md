...
Where:
- `CC`: Cyclomatic complexity of function/method `m` (`≥ 1`).
- `cov`: Test coverage of `m` as a fraction between `0.0` (0%) and `1.0` (100%).

---

## Metric Behavior & Interpretation

The cubic penalty term `(1 − cov)³` heavily penalizes high complexity when test coverage is absent or low:
...
| **20** (Complex method) | 75% | **26.3** | Acceptable, requires `≥ 75%` coverage |
...
|:---|:---|:---|
| `≤ 30` | **Low / Acceptable** | Code is reasonably simple or sufficiently covered by tests. Maintain coverage. |
...
> [!NOTE]
> When `CC > 30`, no amount of test coverage can bring CRAP under the standard threshold of 30. Such functions **must** be broken down into smaller components.
